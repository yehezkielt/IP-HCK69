...
class ShoesController {
  ...
  static async add(req, res, next) {
    try {
      ...
      const base64Url = `data:${req.file.mimetype};base64,${base64Img}`;
      ...
      res.status(201).json({ message: `Success added ${req.body.title}'s shoes` });
    } catch (error) {
      next(error);
    }
  }
  static async update(req, res, next) {
    try {
        const { id } = req.params;
        const dataShoes = await Product.findByPk(id);
        ...
        res.status(200).json({ message: `Success updated ${req.body.title}'s shoes` });
    } catch (error) {
        next(error);
    }
}
